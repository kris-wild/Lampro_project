...
########
## Packages
pacman::p_load(brms,bayesplot,lme4,tidyverse, latex2exp)
...
####################
model1 <- brms::brm(CTmax ~ 1, data = CT.data.raw, family = gaussian(), chains = 4, iter = 5000, thin = 5, warmup = 1000, cores = 4)
...
####################
model2 <- brms::brm(CTmax ~ sex, data = CT.data.raw, family = gaussian(), chains = 4, iter = 5000, thin = 5, warmup = 1000, cores = 4)
model2 <- add_criterion(model2, c("waic", "loo"))
# Checks and plots
m2.draws <- as.array(model2)
mcmc_acf(m2.draws,  pars = c("b_Intercept", "b_sexm"), lags =10)
plot(model2)
# Residuals
...
####################
model3 <- brms::brm(CTmax ~ scale(mass) + sex, data = CT.data.raw, family = gaussian(), chains = 4, iter = 5000, thin = 5, warmup = 1000, cores = 4)
model3 <- add_criterion(model3, c("waic", "loo"))
# Checks and plots
m3.draws <- as.array(model3)
mcmc_acf(m3.draws,  pars = c("b_Intercept", "b_sexm", "b_scalemass"), lags =10)
plot(model3)
# Residuals
resid.3 <- residuals_brms(model3, CT.data.raw)
hist(resid.3)
# R3 and summary of full model
bayes_R2(model3)
summary(model3)
# save model
...
####################
model4 <- brms::brm(CTmax ~ scale(mass) + sex + temp, data = CT.data.raw, family = gaussian(), chains = 4, iter = 5000, thin = 5, warmup = 1000, cores = 4)
model4 <- add_criterion(model4, c("waic", "loo"))
# Checks and plots
m4.draws <- as.array(model4)
mcmc_acf(m4.draws,  pars = c("b_Intercept","b_sexm", "b_scalemass", "b_temp28"), lags =10)
plot(model4)
# Residuals
...
####################
model5 <- brms::brm(CTmax ~ scale(mass) + sex + treat, data = CT.data.raw, family = gaussian(), chains = 4, iter = 5000, thin = 5, warmup = 1000, cores = 4)
model5 <- add_criterion(model5, c("waic", "loo"))
# Checks and plots
m5.draws <- as.array(model5)
mcmc_acf(m5.draws,  pars = c("b_Intercept","b_sexm", "b_scalemass", "b_treatC"), lags =10)
plot(model5)
# Residuals
...
####################
# Model 6 & checks(lags,residuals, r2, summary)
####################
model6 <- brms::brm(CTmax ~ scale(mass) + sex + temp + treat + temp:treat, data = CT.data.raw, family = gaussian(), chains = 4, iter = 5000, thin = 5, warmup = 1000, cores = 4)
summary(model6) 
model6 <- add_criterion(model6, c("waic", "loo"))
# Checks and plots
m6.draws <- as.array(model6)
mcmc_acf(m6.draws,  pars = c("b_Intercept", "b_sexm", "b_scalemass", "b_temp28","b_treatC", "b_temp28:treatC"), lags =10)
plot(model6)
# Residuals
resid.6 <- residuals_brms(model6, CT.data.raw)
hist(resid.6)
# R2 and summary of full model
bayes_R2(model6)
summary(model6)
